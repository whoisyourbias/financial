...
- [ ] `TP-PAY-001..003`, `TP-WIRE-001`, `TP-WIRE-003`, `TP-WIRE-005..006`, `TP-STATE-001`, `SBX-001`의 적용 여부가 매트릭스에 기록되고 `TP-WIRE-002/004`의 후속 소유 프로젝트가 명시된다.
- [ ] 세션 생성 → 승인 → paymentKey/orderId 조회가 샘플 상점에서 동작한다.
- [ ] 공개 경로와 `/sandbox/**` 경로가 명확히 분리된다.
- [ ] 합성 시크릿 Basic 인증의 정상·누락·형식 오류·잘못된 키가 테스트되고, 수명주기·역할 기능은 프로젝트 05 범위로 표시된다.
