...
- 환경 변수로 주입한 합성 테스트 시크릿으로 `Basic base64(secret:)` 공개 wire 형식만 검증; MID·환경 격리와 키 저장·회전·역할 관리는 `INT-AUTH-001`로 분리해 프로젝트 05로 이관

프로젝트 01의 필수 계약 ID는 `TP-PAY-001..003`, `TP-WIRE-001`, `TP-WIRE-003`, `TP-WIRE-005..006`, `TP-STATE-001`, `SBX-001`입니다. `TP-WIRE-002` 공개 멱등성은 프로젝트 02, `INT-AUTH-001` credential 수명주기·격리는 프로젝트 05, `TP-WIRE-004` 버전 호환성은 프로젝트 07의 필수 종료 조건에서 검증합니다.
...
저장·전송 Money는 기준선의 `long amountMinor` 최소 화폐 단위로 고정합니다. 나눗셈이 필요한 이후 수수료 계산에서만 이름 있는 `BigDecimal` 반올림 정책을 사용하며 `double`과 `float`는 사용하지 않습니다.
...
2. 최소 Basic 인증 경계와 결제 세션·Payment 집계를 구현합니다.
...
- `long amountMinor` 저장 계약과 나눗셈 연산의 `BigDecimal` 반올림 경계
...
- 프로젝트 01의 `paymentKey` 중복 승인 방어가 공개 `Idempotency-Key` 응답 재생 계약이 아님을 고정한 테스트
- 분개 균형 속성 테스트
- 트랜잭션 실패 시 Payment와 원장이 함께 롤백되는 테스트
- 샘플 상점 실행 기록
- `projects/01-ledger-core/evidence/` manifest와 재현 명령
