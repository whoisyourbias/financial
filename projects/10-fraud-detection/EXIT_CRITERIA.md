...
- [ ] 고정 fixture의 입력 품질 저하가 탐지되고 사람 override가 감사된다.
- [ ] 결과가 `NORMAL_REVIEW/PRIORITY_REVIEW/ESCALATE` 우선순위로만 노출되며 결제 command 허용·차단에는 연결되지 않는다.
- [ ] 합성 데이터 결과를 실제 고객 효과로 일반화하지 않는다.

## 공통 증거 게이트

- [ ] `projects/10-fraud-detection/evidence/MANIFEST.md`가 `docs/EVIDENCE_POLICY.md`의 공통 manifest 계약과 `evidenceCheck`를 통과한다.
